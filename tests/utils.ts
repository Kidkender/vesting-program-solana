...
export const createMint = async (provider: anchor.AnchorProvider, decimals: number): Promise<PublicKey> => {
    const tokenMint = new anchor.web3.Keypair();
    const lamportForMint = await provider.connection.getMinimumBalanceForRentExemption(spl.MintLayout.span);
...
export const createUserAndATA = async (provider: anchor.AnchorProvider, mint: PublicKey): Promise<[Keypair, PublicKey]> => {
    const user = Keypair.generate()
    let token_airdrop = await provider.connection.requestAirdrop(user.publicKey, 10 * LAMPORTS_PER_SOL);
...
export const getTokenBalance = async (tokenAccount: PublicKey, provider: anchor.AnchorProvider) => {
    const account = await provider.connection.getTokenAccountBalance(tokenAccount).catch(() => null);
    if (!account) {
        console.log("Token account not found:", tokenAccount.toBase58());
        return 0;
    }
    return parseInt(account.value.uiAmount);
}



export async function createPDA(seeds: Buffer[], programId: PublicKey): Promise<[PublicKey, number]> {
